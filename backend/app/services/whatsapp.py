...
def configuration():
    ...
    if not settings.OPERATIONS_API_KEY: missing.append('OPERATIONS_API_KEY')
...
async def send_one(db,delivery_id,client):
    ...
    db.refresh(row)
    body={'From':settings.TWILIO_WHATSAPP_FROM,'To':'whatsapp:'+recipient.phone,'ContentSid':template,'ContentVariables':json.dumps({k:v for k,v in row.payload.items() if not k.startswith('_')})}
    try:
        ...
        if response.status_code==429:
            ...
            row.status='FAILED';row.error=f'Twilio rejected request (HTTP {response.status_code}); check sender, template and recipient in console'
        else:
