...
def get_or_load_model() -> dict:
    ...
    if _LOADED_MODEL_ARTIFACT is not None:
        return _LOADED_MODEL_ARTIFACT

    artifact_path = os.path.join(settings.MODEL_DIR, settings.MODEL_FILE_NAME)
    if not os.path.exists(artifact_path):
        print(f"Model artifact not found at {artifact_path}. Training new model on startup...")
        train_and_save_model(artifact_path)

    _LOADED_MODEL_ARTIFACT = joblib.load(artifact_path)
    return _LOADED_MODEL_ARTIFACT
