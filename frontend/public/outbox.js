  ...
  async function sync(token, foreground = false, force = false) {
    return lock(async () => {
      ...
      for (const item of await all()) {
        ...
        try {
          const response = await fetch(item.endpoint, {method: 'POST', headers: {'Content-Type': 'application/json', ...(key ? {'X-Operations-Key': key} : {})}, body: JSON.stringify(item.report), signal: AbortSignal.timeout(45000)});
          if (!response.ok) {
